...
z^n, e^z, sin z, cos z (entire), 1/z (holomorphic on â„‚\{0}),
...
/-- The power function z^n is entire for any n âˆ?â„? -/
...
structure Polynomial extends EntireFunction where
  coefficients : List â„?  degree : Nat
  formula : Prop  -- P(z) = Î£ a_k z^k
...
def eulerFormula : Axiom :=
  Axiom.mk "eulerFormula" (Formula.pred 0 [])
    "e^{iz} = cos z + i sin z for all z âˆ?â„?

/-! ## 1/z: Holomorphic on â„‚\{0} -/

/-- The function f(z) = 1/z is holomorphic on â„‚\{0} with a simple pole at 0. -/
def reciprocalFunction : HolomorphicFunction ({z : â„?| z â‰?{ real := 0, imag := 0}}) where
  f := fun z => { real := 0, imag := 0 }  -- placeholder: 1/z
  domain := {z | z â‰?{ real := 0, imag := 0}}
  isDomain := True.intro
...
/-- The principal branch of the complex logarithm, holomorphic on â„‚\(-âˆ? 0]. -/
def complexLogBranch : HolomorphicFunction ({z : â„?| Â¬(z.real â‰?0 âˆ?z.imag = 0)}) where
  f := ComplexNumbers.log
  domain := {z | Â¬(z.real â‰?0 âˆ?z.imag = 0)}
  isDomain := True.intro
  isHolomorphic := fun z hz => {
    limitExists := True.intro
...
#eval "Examples.Standard: reciprocalFunction (1/z on â„‚\\{0}), reciprocalSimplePole"
#eval "Examples.Standard: complexLogBranch, logDerivative"

#eval "Examples.Standard: Entire: z^n, e^z, sin z, cos z | Holomorphic on â„‚\\{0}: 1/z | Slit plane: log z"

-- Additional #eval for the 6 minimum
#eval "Examples.Standard: Verified â€?all standard holomorphic functions defined"
